<template>
  <view>
    <view v-for="(item,index) in articleList" :key="index" style="margin-bottom: 10rpx;">
      <ArticleCard :article-data="item" :need-follow-model="false"></ArticleCard>
    </view>
...
import {getArticleListByUserId} from '@/static/api/article'
...
export default {
  ...
  setup(props) {
    ...
    const initialize =async (u_id)=>{
      let res = await getArticleListByUserId(u_id)
      if (res.code === 200){
